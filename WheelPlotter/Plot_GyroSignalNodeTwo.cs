public class Plot_GyroSignalNodeTwo
{
    public Plot_GyroSignalNodeTwo(List<Sample> record)
    {
        double[] x_One = record.Select(r => r.NodeTwo_Value).ToArray();
        double[] x_Two = record.Select(r => r.NodeTwo_SmoothedValue).ToArray();

        var plt = new ScottPlot.Plot(1600, 900);
        plt.XLabel("Time [s]");
        plt.Title($"AngularVelocity raw/smoothed Node Two");
